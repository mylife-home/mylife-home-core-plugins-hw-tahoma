...
module.exports = class RollerShutter {
  constructor(config) {
    ...
    this._deviceOnlineCallback      = (value) => (console.log('online', value) || (this.online = value ? 'on' : 'off'));
    this._deviceExecutingCallback    = (value) => (console.log('exec', value) || (this.exec = value ? 'on' : 'off'));
  ...
  _stateChanged() {
    const value = 100 - this._device.getState(States.STATE_CLOSURE);
    if(value !== this.value) {
      this.value = value;
    }
  }
